...
datadog_df <- read.csv("../data/datadog_species_rich.csv")
...
# Richness graphs
To look at species richness among all the different habitat types, we will create a set of boxplots using the data collected by DataDog. Remember that we need to make sure that the habitat variable is understood in R to be a factor (variable with categories). 

```{r richness barplot, warning=FALSE, message=FALSE}

#factor levels for habitat variable
datadog_df$habitat <- factor(hab$habitat,levels=c("forest","shrubs","fragment","tree","fence","pasture"))

barplot(datadog_df$rich, datadog_df$habitat)
...
############ Code for bar graph with confidence intervals
```{r}
library(sciplot)
...
(1) Create a boxplot for elevation by habitat.
...
**Questions:** What is the story that is going on in this boxplot? 
What is the story that is going on in this bar graph?
