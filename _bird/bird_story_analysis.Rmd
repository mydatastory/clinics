...
(1) Create a bar graph **another variable?**
```{r bar graph student exercise, warning=FALSE, message=FALSE}

# #read in data
# statcat_df <- read.csv("../data/statcat.csv")
# 
# #table of ??
# counts_var <- table(statcat_df$var)
# counts_var
# 
# #bar graph of table
# barplot(counts_var,
...
# boxplot(elevation ~ habitat, data=statcat_df,
...
forest_df <- filter(cat_species_df, habitat == "forest") #creating a subset of forest habitat only

plot(rich ~ elevation, data=forest_df,
            main = "Elevation by Species Richness Graph",
            xlab = "Elevation",
            ylab = "Species Richness")
...
mod.elev <- lm(rich~elevation,data=forest_df)
summary(mod.elev)

```

**Comment:** Not a great model, another variable that would work better? Should we look at fragments for habitat type instead?
