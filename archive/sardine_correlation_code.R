...
adj_landings_df <- landings_df
adj_landings_df$year <- adj_landings_df$year - 10

analysis_df <- merge(avg_temps_df, adj_landings_df, by = 'year')
analysis_df <- subset(analysis_df, year > 1927 & year < 1945)
...
cor(analysis_df$monterey_tons, analysis_df$avg_temp)
