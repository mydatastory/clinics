...
![](../assets/web_lesson_graphics/wendy/head_thinking.svg)
...
![](../assets/web_lesson_graphics/datadog/body_laserpointer.svg)
...
library(dplyr) #load dplyr package (make sure it is installed first)

setwd("C:/Users/cbolc/Desktop/stories/data")
...
# Data cleaning
## Pl. 7
![](../assets/web_lesson_graphics/datadog/body_pointing.svg)
...
**DataDog:** Using the subset command and then grouping the data to calculate the mean for each year is the way that I write my code to be accurate and clean. I never want to loose important data points when data cleaning!
...
temps_df <- subset(temps_df, year < 1968) #create another subset for years less than 1968
...
# Graphing the data
## Pl. 9
![](../assets/web_lesson_graphics/datadog/body_whiteboard.svg)
...
## Pl. 10
![](../assets/web_lesson_graphics/datadog/body_triumph.svg)
...
![](../assets/web_lesson_graphics/datadog/body_snarky.svg)
...
![](../assets/web_lesson_graphics/wendy/head_happy.svg)
...
![](../assets/web_lesson_graphics/statcat/head_considering.svg)
...
![](../assets/web_lesson_graphics/statcat/head_static.svg)

**StatCat:** Let's try a smoothing technique called the loess line for the temperature dataset. To do this we need to create a temperature model, called loess. Then we can generate predicted values using that model, and lay down a smoothing line on top of the graph. 
...
plot(avg_temps_df$year, avg_temps_df$avg_temp, #same plot as above with raw data                  
     type = "l",
     col  = "blue",
     main = "",
     xlim = c(1915, 1970),
...
lines(temp_line, x = avg_temps_df$year, col = 'red') #adds a line in red with the predicted values 
...
plot(x = analysis_df_raw$avg_temp, y = analysis_df_raw$monterey_tons, #graph the dataset
...
plot(x = analysis_df_raw$avg_temp, y = analysis_df_raw$monterey_tons, #plot of the dataset
     xlab = 'Surface Temp (Celsius)',
     ylab = 'Tons')

abline(fit_raw, col = 'red') #add the linear regression line to the plot
...
landing_df$year <- 1917:1967 #add a column in each dataset with the year
temp_df$year    <- 1917:1967

analysis_df_smooth <- merge(temp_df, landing_df, by = 'year') #merge datasets by year
analysis_df_smooth <- subset(analysis_df_smooth, landings > 500) #subset landings greater than 500 tons
analysis_df_smooth <- subset(analysis_df_smooth, year < 1954) #subset year less than 1954

plot(x = analysis_df_smooth$temps, y = analysis_df_smooth$landings,#plot the dataset
     xlab = 'Surface Temp (Celsius)',
     ylab = 'Tons')
