...
8. Ethics in Hypothesis Testing - P-Hacking
...
```{r}
setwd("/Users/ScottCohen/OneDrive - University of Florida/MyDataStory") #set working directory, this 'tells' R where the file path of where the data is located.
...
Pl. 19
**DataDog:** Got it! All coming to plan...Wow...theres really is over 4,000 census tracts in the Census dataset and 442 measurements in the other. I think we need develop a plan of attack! Let's develop an analysis plan!

Pl. 20
*DataDog steping over to a chalk/white board*
...
Pl. 25
**DataDog:** Since the Illudium Phosdex file contains a random sample of 250 census tracts, but we only want those that are less than 200 miles from the Fountain of Youth, we will use a natural join the Illudium Phosdex data frame and the FLcensus subset datframe using the merge() function. GEOID is going to be the variable we basing the matching on, this is because it is the unique identifier in both datasets.
...
Pl 26.
*DataDog discussing with herself*
**DataDog:** Now that I have everything in one dataset, I can use a whole set of functions to calculate descriptive statistics among our three variables of interest. This is important because these numbers will help us decide the appropriate inferential statistical methods based on these distinctions, and interpret the results.
...
Pl. 11
*DataDog pulls up the scatterplots on the screen*
**DataDog:** Here kitty, kitty...  I've got something to show you.
**StatCat:** Hey - who you calling kitty?

Pl. 
*DataDog explaining results to StatCat, StatCat has his bags packed to go travel to Fountain of Youth*
**DataDog:** Look! There looks like the is some pattern with your Illudium Phosdex measurements and distance from the Fountain of Youth! But it does not appear to be the case with your age measuresments using the Census data.
...
**StatCat:** Well I am telling you this story is true! There must be the wrong data. There has to be another dataset somewhere with the right age measurements...
...
Pl.
*StatCat in a dark basement, digging through dusty, old UNIVAC systems*
**StatCat:** I must defend my family's honor, I know there is an old reference down here somewhere.

Pl.
*StatCat continues to running UNIVAC systems in search for code, eyes widen when he finds what he is looking*
